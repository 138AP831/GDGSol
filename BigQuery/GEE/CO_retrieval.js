...
// Function to handle user click and extract CO data
var handleClick = function(coords) {
  ...
  // Sentinel-5P image collection for CO
  var sentinel5PCO = ee.ImageCollection("COPERNICUS/S5P/OFFL/L3_CO")
  ...
  var monthlyCOAtPoint = monthlyCOData.map(function(image) {
    var COValue = image.reduceRegion({
  ...
  var COFeatureCollection = monthlyCOAtPoint.map(function(image) {
    return ee.Feature(null, {
      'date': image.get('system:index'),
      'CO_value': image.get('CO_value'),
  ...
  Export.table.toDrive({
    collection: COFeatureCollection,
    description: 'CO_Emissions_CSV',
    fileFormat: 'CSV',
    folder: 'Gas_Monitoring',
    fileNamePrefix: 'CO_Emissions'
  });

  Export.table.toDrive({
    collection: COFeatureCollection,
    description: 'CO_Emissions_JSON',
    fileFormat: 'GeoJSON',
    folder: 'Gas_Monitoring',
    fileNamePrefix: 'CO_Emissions'
  });
